...
namespace GameBrains.Entities.V0
{
    public class Agent : Entity
    {
        ...
        // TODO: Add support for global, local, external messages queues.
        [SerializeField] protected MessageQueue messageQueue;
        [SerializeField] protected bool showThots;
        ...
        public override void Awake()
        {
            ...
            #region Disable showing messages beside weebles for now
            
            var thotsTransform = transform.Find("WeebleCanvas/Thots");
            if (thotsTransform && !showThots)
            {
                thotsTransform.gameObject.SetActive(false);
            }
            
            #endregion Disable showing messages beside weebles for now
            
            switch (MotorType)
            {
                // TODO for A1 (optional): Add case for other motor types (e.g., TransformTranslate)
        ...
        protected virtual void Think(float deltaTime)
        {
            ...
            {
                // TODO for A1 (optional): Should we deal with inprogress actions or just drop them?
        ...
        {
            // Replace existing actions with new ones, Add to existing actions, or
            // Merge new actions with old (updating parameters).
            
            // TODO for A1 (optional): Which approach is best for this application?
        ...
        public bool DisplayMessage(string message, Color messageColor)
        {
            if (!messageQueue) { return false; }
            messageQueue.AddMessage(message, messageColor);
            return true;
        }
    }
}